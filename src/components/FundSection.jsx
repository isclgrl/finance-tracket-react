...
// 1. Agregamos excludedFundIds y toggleFundInclusion a las props
const FundSection = ({ 
  funds, 
  currentPeriod, 
  showAddFund, 
  ...
  onCreateFund, 
  onTopUp,
  excludedFundIds,      // <--- NUEVO
  toggleFundInclusion   // <--- NUEVO
}) => {
  
  // Ya NO usamos useState aquí dentro. Lo borramos.

  const totalBalance = funds.reduce((acc, fund) => {
    if (excludedFundIds.includes(fund.id)) return acc;
    return acc + fund.balance;
  }, 0);

  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      {/* ... (El bloque del Total y el Formulario quedan IGUAL) ... */}
      
      {/* Solo cambiamos cómo se llama a la función en el map */}
      <div className="bg-slate-500 text-white p-6 rounded-xl shadow-lg md:col-span-3 flex justify-between items-center">
         {/* ... contenido del header igual ... */}
          <div>
            <p className="text-gray-300 text-sm font-medium uppercase">Fondo Total (Seleccionado)</p>
       ...
       {/* Formulario Nueva Bolsa (IGUAL QUE ANTES) */}
      ...
      {funds.map(fund => {
        // Usamos la prop que viene desde App
        const isIncluded = !excludedFundIds.includes(fund.id);
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <input 
                    type="checkbox" 
                    checked={isIncluded} 
                    onChange={() => toggleFundInclusion(fund.id)} // <--- Usamos la función del prop
                    className="w-4 h-4 text-blue-600 rounded cursor-pointer"
                  />
                  <h3 className="text-gray-500 font-bold text-xs uppercase cursor-pointer" onClick={() => toggleFundInclusion(fund.id)}>
                    {fund.name}
                  </h3>
                </div>
                {/* ... Resto igual ... */}
